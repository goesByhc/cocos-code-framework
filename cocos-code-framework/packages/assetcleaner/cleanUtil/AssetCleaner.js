...
let AssetCleaner = {
    ...
    start(sourceFile) {
        ...
        this.getSortedResult(noBindMap);
        this.getSortedResult(noLoadMap);

        let noBindFile = this.mapToArr(noBindMap);
        let noLoadFile = this.mapToArr(noLoadMap);

        return [noBindFile, noLoadFile];
    },

    //将map转化为数组
    mapToArr(fileMap) {
        let result = [];
        fileMap.forEach(fileMap => {
            let fileArr = []
            for (let i = 0; i < fileMap.length; ++i) {
                let strs = fileMap[i].path.split('.');
                let ext = '.' + strs[strs.length - 1];
                let file = { path: fileMap[i].path, size: Utils.byte2KbStr(fileMap[i].size), ext: ext };
                fileArr.push(file);
            }
            result.push(fileArr);
        });
    ...
    getFileInfo(fileArr) {
        ...
        content += '\n';
        outStr += fileArr.length + '个';
        outStr += ', 占用空间=' + totalSize + 'KB';
        outStr += content;
        return outStr;
    },

    //删除未引用的资源
    deleteNoBindFile(outStr, outMap, srcDir, isDelete) {
        let content = '';
