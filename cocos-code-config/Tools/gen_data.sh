...
xlsxPath=$script_path/../Design/
protoDataFolder=$script_path/../DataAssets
archiveFolder=$protoDataFolder/Archive
tsGaneratorTool=$script_path/TSConfigGenerator/generate_config_all-macos

rm -rf $targetProtoFolder
mkdir $targetProtoFolder

rm -rf $protoDataFolder
mkdir $protoDataFolder
mkdir $archiveFolder
...
#gen typescript
sh $script_path/gen_data_ts.sh

#gen go   
#go的生成文件依赖TS
sh $script_path/gen_data_go.sh
